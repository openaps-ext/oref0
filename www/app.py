...
NS_URL = config.get('device "ns"', 'args').split(' ')[1]
NS_TOKEN = config.get('device "ns"', 'args').split(' ')[2]
MEDTRONIC_PUMP_ID = config.get('device "pump"', 'serial')
...
def pumphistory():
    json_url = os.path.join(myopenaps_dir + "monitor/pumphistory-zoned.json")
    data = json.load(open(json_url))
    return jsonify(data)
